...
public class AdminController {
    ...
    public ResponseEntity<?> updateDriver(@PathVariable String userId, @RequestBody User driver) {
        try {
            ...
            if (existingDriver == null || !"driver".equalsIgnoreCase(existingDriver.getRole())) {
                Map<String, Object> response = new HashMap<>();
                response.put("success", false);
                response.put("message", "Driver not found");
                return ResponseEntity.status(404).body(response);
            ...
            driver.setRole("driver"); // Ensure role remains driver
    ...
    public ResponseEntity<?> getAdminStatistics() {
        try {
            ...
            long driverCount = allUsers.stream().filter(u -> "driver".equalsIgnoreCase(u.getRole())).count();
            long staffCount = allUsers.stream().filter(u -> "staff".equalsIgnoreCase(u.getRole())).count();
            long activeDrivers = allUsers.stream()
                .filter(u -> "driver".equalsIgnoreCase(u.getRole()) && "active".equalsIgnoreCase(u.getStatus()))
                .count();
